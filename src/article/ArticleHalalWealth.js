  ...
  const downloadPDF = () => {
    ...
    additionalText.innerHTML = `
      <p style="color: white; font-size: 14px; margin-top:10px;">
        <strong>Host</strong>: <span style="font-size: 14px; margin-top:10px;">Taofiq Akinpeju</span>
      </p>
      <p style="color: white; font-size: 14px; margin-top:10px;">
        <strong>Speakers</strong>: <span style="font-size: 14px; margin-top:10px;">Kayode Quadri | Dr Kazeem Salam | Ust. Taofeeq AbdusSalaam</span>
      </p>
      <p style="color: white; font-size: 14px; margin-top:10px;">
        <strong>Summarised by</strong>: <span style="font-size: 14px; margin-top:10px;">Abu Muhammad Oladitan Saliu</span>
      </p>
      <p style="color: white; font-size: 14px; margin-top:10px;">
        <strong>Date</strong>: <span style="font-size: 14px; margin-top:10px;">17th December, 2022</span>
      </p>
    `;
...
Date: <span style={{ fontSize: '12px', color:"gray"}}>17th December, 2022</span>
        ...
        <button
          onClick={downloadPDF}
          className="bg-blue-500 gap-2 flex justify-start align-start text-white px-4 py-2 rounded mt-4 hover:bg-blue-700 transition"
        >
  ...
  const downcontent = (
    <section className="block sm:hidden -mb-5 mt-2 py-4 sm:p-4 px-3  content">
      <div className="p-1 w-64 rounded text-black">
  ...
  const content = (
    <section id="content" className="text-black sm:mb-20 mb-5">
      {/* Original Content (unchanged) */}
      <section>
        <h1 className="font-bold text-blue-600 -mb-2 sm:mb-0 lg:w-10/12 lg:text-4xl md:w-96 md:text-2xl text-xl px-4">
        ...
        <p className="text-sm text-black mt-3 sm:mb-10 mb-5 text-center">Complete video</p>
      </section>
      <section>
                <h2 className='text-black mb-0 lg:w-10/12 md:w-96 w-80 '>I'm happy to offer you a minified summary and personal additional notes to one of the finest events held about generating halal income this year. I see it as a duty because Muslims really need it. </h2>
                <p className='text-black text-sm mt-4 sm:mb-7 mb-5'>Let's start...</p>
                <p className='text-black sm:mb-10 mb-5 lg:w-10/12 md:w-96 w-80 '>I congratulate you for finding this piece of work and wanting to read it. It's worth the time. You will be better off.
                </p>
            </section>
            <section>
            ...
            <p className='text-black mt-3 mb-10' style={{fontSize:"11px"}} >Cited by Dr Kazeem Salam (Yoruba proverb for the wise)</p>
            <h1 className='text-black mb-5 lg:w-10/12 md:w-96 w-80 text-sm'>* WORKING FOR SALARY/WAGES IS FOR EATING FOOD AND PAYING BILLS ONLY (most times), BUT BUSINESS/PROFIT IS TO MAKE YOU RICH AND WEALTHY! Be wise!
            </h1>
            <div className='translate-x-2'>
            <p className='text-black mb-5 lg:w-10/12 md:w-96 w-72 text-justify'>However, migrating from being an employee to an employer should be gradual. I will advise you to gather enough money, education (financial education), experience, materials and confidence needed before you switch. It's also true that not all businesses need a lot of experience and money to start, you will learn and acquire more funds as you progress. Don't allow anything to hold you back if you're convinced and determined. Your readiness, having a sound business idea and proper planning, all crowned with prayer are more important than the capital for a business to be successful. Even having a sound idea with proper planning will attract the right funding.
            </p>
            <p className='text-black mb-2 lg:w-10/12 md:w-96 w-72 text-sm'>
            1. It's about you identifying a problem, finding a solution, money will always follow - Kayode Quadri.
            
            </p>
            <p className='text-black mb-2 lg:w-10/12 md:w-96 w-72 text-sm text-justify'>
    ...
    <div className='grid lg:grid-cols-5 lg:mx-28 md:mx-10 md:-translate-x-6 lg:-translate-x-16 my-5 md:grid-cols-3 grid-cols-2 gap-3 text-center mx-5 -translate-x-'>
        ...
        <img src={partner} alt="imagesave" className='w-60 h-20 lg:w-32 md:w-32 rounded '/>
            <p className='text-black' style={{fontSize:"10px"}}>Partnership</p> 
        </div>
        <div>
        <img src={crownfunding} alt="imagesave" className='w-60 h-20 lg:w-36 md:w-32 rounded '/>
        <p className='text-black' style={{fontSize:"10px"}}>Crowdfunding</p>
        </div>
        <div>
        <img src={business} alt="imagesave" className='w-60 h-20 lg:w-36 md:w-32 rounded '/>
        <p className='text-black' style={{fontSize:"10px"}}>Business grant</p>
        </div>
        <div>
        <img src={investing} alt="imagesave" className='w-60 h-20 lg:w-36 md:w-32 rounded '/>
        <p className='text-black' style={{fontSize:"10px"}}>Angel investors</p>
        </div>
        <div>
        <img src={interest} alt="imagesave" className='w-60 h-20 lg:w-36 md:w-32 rounded '/>
        <p className='text-black' style={{fontSize:"10px"}}>Non-interest loan</p>
        </div>
        <div>
        <img src={capitalist} alt="imagesave" className='w-60 h-20 lg:w-36 md:w-32 rounded '/>
        <p className='text-black' style={{fontSize:"12px"}}>Venture capitalist</p>
        </div>
        <div>
        <img src={Incubators} alt="imagesave" className='w-60 h-20 lg:w-36 md:w-32 rounded '/>
        <p className='text-black' style={{fontSize:"10px"}}>Business Incubators</p>
        </div>
        <div>
        <img src={incentives} alt="imagesave" className='w-60 h-20 lg:w-36 md:w-32 rounded '/>
        <p className='text-black' style={{fontSize:"10px"}}>Government incentives</p>
        </div>
        <div>
        <img src={asset} alt="imagesave" className='w-60 h-20 lg:w-36 md:w-32 rounded '/>
        <p className='text-black' style={{fontSize:"10px"}}>Work to raise money/sell your assets</p>
        </div>
        <div>
        <img src={connections} alt="imagesave" className='w-60 h-20 lg:w-36 md:w-32 rounded '/>
        <p className='text-black' style={{fontSize:"10px"}}>Friends, family, network and connections</p>
        </div>
    </div>
        <p className='text-black my-3 lg:w-10/12 md:w-96 w-72 text-sm text-justify'>(Learn about each method and know the best ways to tap into them.)</p>
<div className='lg:translate-x-12 md:translate-x-3'>
        <p className='text-black mb-2 lg:w-9/12 md:w-96 w-72 text-sm text-justify'>
        ...
        * So baarokallohu feek if you can achieve it. If you can, please go for it. Don't settle for a life that's less than the one you're capable of living. It's worth it and the society needs you.
        ...
        <p className='text-black mb-3 lg:w-9/12 md:w-96 w-72 text-sm mt-5 text-justify'>
        <h2 className='text-sm sm:text-sm font-bold mb-3 text-blue-600'>📜 Don't forget: </h2> 
        <p className='mb-3 text-black'>..ISE OMO ALASE JE, OWO OMO ALASELA!</p>
        <h2 className='text-sm sm:text-sm font-bold mb-1 text-blue-600'>📜 Remember also: </h2>  
        <p className='mb-5 text-black'>"Complaint has never changed anything, but proper action always does" - Oladitan Saliu</p>
        <p className='mb-4 text-black'>☘Wishing you all a successful, happy, righteous and fun-filled life.</p>
        <p className='mb-4 text-black'>🌲Ultimately, may Allah grant us all Al-jannah Al-Firdaws.</p>
        <p className='mb-4 text-black'>🙏🏾Please pardon me wherever I might have erred, all perfection belongs to Allah.</p>
        <p className='mb-4 text-black'>📱📲Remember to share. It's a choice. But you can choose to benefit others and earn some rewards. I've done my quota.</p>
        <p className='mb-4 text-black'>🤝 Congratulations and bye for now. Later in shaa Allah.</p>
        <p className=' sm:-mb-6 text-black'>🪴 Ma'a as-salaamah...</p>
        </p>
        <button
          onClick={downloadPDF}
  ...
  return (
    ...
    <section className="flex w-full h-full bg-white justify-center items-center content">
        <div className="flex w-full h-full flex-row justify-around md:mx-10 lg:mx-24 sm:p-4 p-2 content lg:translate-x-16">
        {content}
